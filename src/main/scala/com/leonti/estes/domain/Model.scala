...
case class Dish(@Key("_id") id: CompositeId, name: String, ingredients: List[Ingredient], selectedIngredients: List[Ingredient], menus: List[String], price: BigDecimal)
case class Order(@Key("_id") id: CompositeId, waiter: Waiter, dishes: List[Dish], submitted: Long, status: String, note: String)
